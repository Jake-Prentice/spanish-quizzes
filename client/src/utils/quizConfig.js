...
import states from "constants/paradigmStates";


export const traverseEachPlainObj = (obj, cb) => {
    for (let value in obj) {
        if (typeof obj[value] === "object") {
            ...
            traverseEachPlainObj(obj[value], cb);
        }
    }
}

export const convertConfigToOptions = (config, options) => { 
    const copiedConfig = JSON.parse(JSON.stringify(config))
    const copiedOptions = options || JSON.parse(JSON.stringify(optionsSchema))
        ...
        for (let value in configsRef) {
            if (typeof configsRef[value] === "object") {
                ...
                if (!configsRef[states[value].next]) {
                    ...
                    if (states[value].next) traverseEachPlainObj(
    ...
    const recurse = (configsRef, optionsRef, currentLevel=0) => {
        ...
        if (intendedLevel !== currentLevel) { 
            ...
            if (currentOption.children) {
                traverseEachPlainObj(currentOption.children, option => 
